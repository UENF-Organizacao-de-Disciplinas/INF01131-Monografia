...
\begin{MyCenteredFigure} \caption{Página de modificação das informações de salas} \label{fig:CRUD_salas_vazias}
...
Seguindo constantemente o conceito de iteratividade da apresentação do sistema, a programação foi marcada por dois conceitos: blocos de funcionalidade marcantes e blocos de funcionalidade apresentadas. O primeiro conceito se refere à grandes grupos de mudanças que estavam relacionadas a um mesmo tópico, ou que resultaram, quando em conjunto, num sistema consideravelmente distinto de como estava antes de as receber. O segundo conceito se refere à apresentação esporádica da situação atual do sistema para quem o iria utilizar, sendo essas versões chamadas de MVP`s (Minimum Viable Product - Mínimo Produto Viável). Nesta seção serão apresentadas as versões do sistema repartidas de forma a apresentar os agrupamentos de mudanças notórias.
...
\subsection{Versão 1.0} \label{subsec:MVP1} % ### 5.5.2. Primeira versão
...
\begin{MyCenteredFigure} \caption{Diagrama da progressão funcionamento da permanência dos dados} \label{fig:API_MVP1}
...
% Explicar isso daqui melhor

\subsection{Versão 2.0} \label{subsec:MVP2} % ### 5.5.3. Segunda versão
...
Utilizando o GitHub Projects, foi organizado uma tabela de tarefas, vista na \autoref{fig:GitHubProjectsTable}, onde foram unificadas as diversas anotações e ideias, antes soltas. A partir disso, foi possível visualizar o que era mais importante e o que poderia ser deixado de lado.
...
\subsubsubsection*{Funcionamento Geral da Permanência dos Dados usando API REST} \label{ssssec:Funcionamento Geral}

Para exemplificar o funcionamento geral da permanência dos dados, consideremos o uso de uma \textit{API REST}, representada pela \autoref{fig:API_MVP2}, utilizando de 4 ``camadas'': o \textbf{\textit{frontend}}, os \textbf{\textit{endpoints}}, as \textbf{funções} de execução e o \textbf{banco de dados}.
...
\begin{MyCenteredFigure} \caption{Paralelo entre os dois métodos de armazenamento dos dados na Versão 2.0} \label{fig:API_MVP2}
...
\begin{MyCenteredFigure} \caption{Logomarcas do sistema} \label{fig:Logomarcas}
  \includegraphics[width=\textwidth]{files/img/2.02!5-desenvolvimento/2.02!5.1.4-sistema/Logomarcas}
\end{MyCenteredFigure}
...
\subsection{Versão 3.0} \label{subsec:MVP3} % ### 5.5.3.
