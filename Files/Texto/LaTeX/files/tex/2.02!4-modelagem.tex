...
\section{Estágios de execução} \label{sec:estagios} % 4.1
...
\begin{CenteredFigure} \caption{Estágios para a obtenção de grade horária ótima} \label{fig:geral}
  ...
  \legend{Fonte: \citeonline{miranda_udpskeduler_2012} - adaptado}
\end{CenteredFigure}
...
\section{Iteração} \label{sec:iteracao} % 4.2
...
\section{Funcionamento} \label{sec:funcionamento} % 4.3
...
\section{API REST} \label{sec:REST} % 4.5
...
\section{Diferença dos trabalhos anteriores} \label{sec:diferenca} % 4.6

Os trabalhos mais próximos ao presente, quanto a instituição são os trabalhos feitos por \citeonline{Sanya2013} e \citeonline{Ricardo2014}, porém, o presente trabalho se distingue em algumas questões.
...
Existem também alguns critérios usados por \citeonline{Sanya2013} e \citeonline{Ricardo2014} que não são considerados no presente trabalho, como a preferência de professores associados e professores contratados, e o desejo por se evitar alocar disciplinas que possuam aulas em horários próximos ou que sobreponham o horário de almoço.
