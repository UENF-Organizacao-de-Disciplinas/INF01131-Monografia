...
O botão de adição de todas as disciplinas foi mantido, e agora, ao ser clicado, cria as turmas com suas características já preenchidas, assim gerando uma solução inicial, porém passível de conflitos. Vale ressaltar que o preenchimento das características, entretanto, não inclui o campo descrição, visto que esta é uma característica única de cada turma e deve ser adicionada manualmente somente em caso de necessidade.

\subsection{Heurística histórica para criação de uma solução inicial} \label{ssec:Heurística}

A solução inicial é obtida através de uma heurística própria que analisa o histórico de criação de turmas, ou seja, todas as turmas, de todos os anos, cadastradas no sistema, com o objetivo de criar todas as turmas necessárias para o semestre. A partir disso, define as características das turmas a serem criadas. Ela é executada assim que o botão de adição de turmas a partir das disciplinas ainda não ofertadas é clicado.
...
Com essas informações o usuário pode escolher criar uma turma para uma \textbf{disciplina em específico}, ou então uma turma para \textbf{cada uma das disciplinas listadas}. Ao criar uma turma para uma \textbf{disciplina específica}, ele inserirá essa uma disciplina em uma lista de turmas a serem criadas. Já ao criar uma turma para \textbf{todas as disciplinas listadas}, o sistema irá inserir todas as disciplinas da listagem em uma lista de turmas a serem criadas.

A partir daí o sistema percorrerá cada uma das disciplinas na listagem das turmas a serem criadas e, se baseando no histórico de turmas cadastradas no sistema, as filtrará para que sejam obtidas apenas as turmas da disciplina em questão.

Dessa listagem filtrada, serão obtidas as informações mais recorrentes sobre as alocações prévia dessa disciplina. Entende-se "informações mais recorrentes" como "o valor que apresenta maior frequência dentre as alocações prévias".

Então, serão obtidos os valores mais frequentes de: quantidade de horários, professor, sala, hora de início e duração. Duas outras informações também obtidas, porém de formas ligeiramente diferentes são: os dias da semana em que a disciplina é ofertada e a demanda estimada.

Para o primeiro caso, baseado na quantidade $H$ de horários oferecidos na semana para tal disciplina, é possível inferir os dias da semana em que a disciplina é ofertada, ao se analisar os $H$ horários mais frequentes.

Já para o segundo caso, a demanda estimada é obtida ao se calcular a média aritmética dos valores presentes nas alocações prévias da disciplina que possuam a informação de demanda.

Após a coleta dessas informações, o sistema criará uma turma para a disciplina em questão contendo as informações obtidas.

% Seu algoritmo é ilustrado no \autoref{code:heuristica}.
