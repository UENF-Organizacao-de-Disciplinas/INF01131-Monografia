...
Uma das formas de se encontrar soluções para problemas complexos é a de relaxar restrições. Ao utilizar do método da programação inteira, \citeonline{Dunke2023} flexibilizam as restrições de seu modelo de programação linear, porém utilizam de uma heurística que corrige as soluções encontradas. De forma similar, \citeonline{Souza2000} menciona o uso de Algoritmos Genéticos que gera frequentemente soluções inviáveis, sendo utilizado então um filtro para que as inviabilidades sejam reparadas.
